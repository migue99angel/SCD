...
void TareaA() { Tarea( "A", milliseconds(250) );  }
...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         sleep_until( ini_sec );
	 milliseconds_f deriva =milliseconds_f(steady_clock::now()-ini_sec);
	 cout<<"La deriva es de: "<< deriva.count() <<endl;
	 if(deriva.count()>0.20){
	    cout<<"La deriva es mayor a 20 milisegundos por lo que se aborta el programa"<<endl;
	    return 1;
		} 
      }
   }
