...
//Autor: Miguel Ángel Posadas Arráez 2ºB
// -----------------------------------------------------------------------------
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
   cout<<"Filosofo "<<id<<" pide permiso al camarero para sentarse"<<endl;
   MPI_Ssend(&valor,1,MPI_INT,id_camarero,etiq_sentarse,MPI_COMM_WORLD);

   MPI_Recv(&valor,1,MPI_INT, id_camarero, etiq_sentarse,MPI_COMM_WORLD,&status);
   ...
   cout<<"Filosofo "<<id<<" informa al camarero de que se levanta"<<endl;
...
void funcion_camarero(){
    int nFilosofos=0,valor, id_filosofo;
    MPI_Status status;
    ...
    while(true){
        ...
        if(status.MPI_TAG==etiq_sentarse){
            id_filosofo = status.MPI_SOURCE;

            MPI_Recv(&valor,1,MPI_INT,id_filosofo,etiq_sentarse, MPI_COMM_WORLD, &status);
            nFilosofos++;

            MPI_Ssend(&valor,1,MPI_INT, id_filosofo,etiq_levantarse,MPI_COMM_WORLD);
            cout<<"Se sienta el filosofo "<<id_filosofo<<", hay "<<nFilosofos<<"sentados"<<endl;
        }
        if(status.MPI_TAG == etiq_levantarse){
            id_filosofo = status.MPI_SOURCE;

            MPI_Recv(&valor,1,MPI_INT,id_filosofo,etiq_levantarse,MPI_COMM_WORLD,&status);
        
            nFilosofos--;

            cout<<"Se levanta el filosofo "<<id_filosofo<<", quedan "<<nFilosofos<<"sentados"<<endl;
        }
      }
    }
